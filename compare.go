...
func calcSat(name string, val *reflect.Value, movie *Movie) (float64, uint) {
	...
	case "BeforeYearExclusive":
		...
		if isBeforeYearExclusive(movie.Year, p.Value) {
			...
			sat = calcDiffPenalty(float64(movie.Year), float64(p.Value), float64(Bounds.MAX_YEAR), float64(p.Weight))
		}
		weight = p.Weight
	case "MaximumAgeRatingInclusive":
		...
		if isMaximumAgeRatingInclusive(movieRating, prefRating) {
			...
			sat = calcDiffPenalty(float64(movieRating), float64(prefRating), float64(Bounds.MIN_RATING), float64(p.Weight))
		}
		weight = p.Weight
	case "ShorterThanExclusive":
		...
		if isShorterThanExclusive(float64(movie.Runtime), prefRuntime) {
			...
			sat = calcDiffPenalty(float64(movie.Runtime), prefRuntime, float64(Bounds.MIN_RUNTIME), float64(p.Weight))
		}
		weight = p.Weight
	case "FavoriteGenre":
		p := pref.(*Preference[string])
